<?php
    ...
    if (isset($_GET['id_obat'])) {
      $id = ($_GET["id_obat"]);
      $result = read_id_obat($id);
      $t_obat_penyakit = read_obat_penyakit($id);
    }
    
    if (isset($_POST['btn-edit-obat'])) {
      
      $list_penyakit = $_POST['list_penyakit'];
      // jalankan query tambah record baru
      $isAddSucceed = update_obat($_POST, $_FILES, $list_penyakit);
      if ($isAddSucceed > 0) {
    ...
    $t_penyakit = read_penyakit();
    $list_obat = read_id_obat($id);
?>
          ...
          <h2>Update Data Obat</h2>
                  ...
                  <div class="container" data-aos="fade-up" style="width: fit-content;">
                    ...
                    <div class="gradient-custom-1 h-100">
                        ...
                        <div class="container">
                            <form action="update_obat.php" method="POST" id="form-add" enctype="multipart/form-data">
                                <input  type="hidden" name="id_obat" id="id_obat" value="<?=$result['id_obat']?>">
                                
                                <div class="mb-3"> <!-- NAMA OBAT-->
                                    <label for="nama_obat" class="form-label">Nama Obat</label>
                                    <input type="text" class="form-control" id="nama_obat" name="nama_obat" value="<?= $list_obat['nama_obat'] ?>">
                                </div> 
                                <div class="mb-3"> <!-- JENIS OBAT -->
                                    <label for="id_jenis_obat">Jenis Obat</label>
                                    <br>
                                    <select class="form-select" aria-label="Category" id="id_jenis_obat" name="id_jenis_obat">
                                ...
                                <div class="mb-3"> <!-- FUNGSI OBAT -->
                                    ...
                                    <?php
                                    while($list_penyakit = mysqli_fetch_assoc($t_penyakit))
                                    {
                                        ?>
                                        <div class="col-md-3">
                                            <input type="checkbox" name="list_penyakit[]"  value="<?=$list_penyakit['id_penyakit']?>"
                                              <?php
                                                foreach ($t_obat_penyakit as $list_obat_penyakit ) {
                                                  if ($list_obat_penyakit['id_penyakit'] == $list_penyakit['id_penyakit']) 
                                                  {?>
                                                  checked
                                                  <?php
                                                  }
                                              ...
                                              ?>
                                            >
                                            <label><?=$list_penyakit['nama_penyakit']?></label> 
                                        </div>
                                    <?php
                                    }
                                    ?>
                                ...
                                <div class="mb-3"> <!-- LINK INFORMASI TENTANG OBAT -->
                                    ...
                                    <input type="url" class="form-control" id="link_obat" name="link_obat" value="<?= $list_obat['link_obat'] ?>">
                                </div><br>
                                
                                <a href="data_obat.php"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button></a>
                                <button type="submit" class="btn btn-info text-white" style="background-color: #3B6E8D" name="btn-edit-obat" id="btn-add" form="form-add">Update Data Obat</button>
                            </form>
